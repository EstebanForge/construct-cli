...
- Learned roots are capped: default 16, configurable as `daemon.max_learned_roots`. Eviction is LRU by `last_used`. Eviction changes the mount set hash, so it recreates on next run; prefer evicting only when the cap is exceeded.
...
Decline memory (2026-09-24, revised 2026-09-25): the consent prompt survives ONLY outside the user's home. Inside `$HOME`, consent is implied by the invocation itself and the root is auto-learned and persisted with no prompt, headless or interactive (Docker parity). The home directory itself is the always-ask regime: mounting it exposes every user file to the sandbox, so EVERY run from `$HOME` prints a danger warning and asks with a default-NO confirm (acceptance is never persisted; the mount rides the run's one-off cwd append, and refusal persists nothing either); `roots add` refuses home so the checkpoint cannot be bypassed. An interactive NO outside `$HOME` persists the folder in `roots.json` under `declined` (path -> timestamp): declined folders never re-prompt, never auto-learn, never enter the single-path mount set, and runs from them fail with `ErrMsbDaemonWorkdirDeclined` plus the `roots add` command to reverse. Declines are subtree exclusions: declining a folder also excludes everything below it, and the declined-run error names the persisted record. Headless runs outside `$HOME` (and inside-home sensitive trees: hidden top-level dirs like ~/.ssh, ~/.aws, ~/.gnupg, ~/.cache, and ~/Library) fail closed without persisting anything.
...
Large-workspace memory (2026-09-25): the entry-count guard (`EvaluateWorkspace`, budget `workspace_max_entries`, default 500000, was 60000 before research showed a single modern JS project can carry 50-150k files and virtiofs on 2020+ NVMe hosts is ~4x the pre-2022 stack) warns and asks once per folder. An interactive YES persists the folder in `roots.json` under `accepted` (path -> timestamp): accepted folders skip the warning and confirm on later runs, subdirectories included. The scan's time budget stays the hard hang guard; a NO persists nothing and asks again; `roots forget <path>` clears an acceptance and `roots list` shows accepted folders in their own section. `max_learned_roots` never counts acceptance entries.
...
- Baked image (2026-09-23, PUBLISHED): ghcr.io/estebanforge/construct-box:latest = sha256:faa0a62938d3 (dual-arch amd64+arm64, ~7.7 GiB per arch, CI run 35875228298); lab identities: docker ac0d1183cd59 8.29 GB, msb store 7.7 GiB sha256:96c46497ce0d. Size history: 24.6 GB brew bake -> 15.6 post-trim -> 8.35 brew exit -> 8.29 after the dart drop, the qmd->rust swap, and the litellm[proxy] pin (+480 MB). Homebrew is GONE: toolchain ships via trixie apt + vendor repos (gh, NodeSource node 24; dart removed 2026-09-22) + go.dev tarball 1.27.1 + rustup 1.98 + a baked mise github: tier (8 release tools, pins in internal/templates/mise-tools.toml). Max layer ~1.74 GB (msb rejects >10 GiB). 17 consecutive lab-matrix passes; every baked tool execute-verified in-build on both arches. Removed tools restore via apt/mise (docs/PACKAGES.md); analysis record in docs/BREW-EXIT.md.
