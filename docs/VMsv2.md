...
Decline memory (2026-09-24, revised same day): the consent prompt survives ONLY outside the user's home. Inside `$HOME`, consent is implied by the invocation itself and the root is auto-learned and persisted with no prompt, headless or interactive (Docker parity); the home directory itself is excluded (learning it would mount the whole home). An interactive NO outside `$HOME` persists the folder in `roots.json` under `declined` (path -> timestamp): declined folders never re-prompt, never auto-learn, never enter the single-path mount set, and runs from them fail with `ErrMsbDaemonWorkdirDeclined` plus the `roots add` command to reverse. Headless runs outside `$HOME` (and inside-home declines, which are now manual-only exclusions) fail closed without persisting anything.
