...
- Baked image (2026-09-22 brew exit, lab): docker uncompressed 8.35 GB, msb store 7.8 GiB (digest 187b7d768356; pre-exit history: 24.6 GB bake -> 15.6 GB post-trim). Homebrew is GONE — toolchain ships via trixie apt + vendor repos (gh, dart, NodeSource node 24) + go.dev tarball 1.27.1 + a baked mise github: tier (8 release tools symlinked into /usr/local/bin, pins in internal/templates/mise-tools.toml). Max layer 1.74 GB (msb rejects >10 GiB). Full baked-stage matrix green post-migration (lab-matrix11): agents at /usr/local/bin, resume skip, sweep, override shadow + fallback, doctor --fix. Removed formulae restore via apt/mise (docs/PACKAGES.md); analysis record in docs/BREW-EXIT.md.
