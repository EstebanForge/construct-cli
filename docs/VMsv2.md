...
- Baked image (2026-09-21, lab): docker uncompressed 24.5 GB (llvm removed by owner decision; max layer 9.61 GB - msb `image load` rejects any single layer over 10 GiB, so the brew install is chunked into size-bounded RUN layers), msb store 23.0 GiB. Fresh-root boot install phase = 6 s (entrypoint_install_phase_sec; no user packages.toml present). Full baked-stage matrix green: agents at /usr/local/bin, sweep, override shadow + fallback, doctor --fix volume cleanup.
