...
- Baked image (2026-09-22 trim, lab): docker uncompressed 15.6 GB (swift, zig, erlang/elixir/gleam + the graphics/llvm chain removed by owner decision; llvm@22 remains via rust; max layer 4.1 GB, msb `image load` rejects any single layer over 10 GiB so the brew install stays chunked), msb store 14.6 GiB (pre-trim: 24.5 GB / 22.8 GiB). Fresh-root boot install phase = 6 s (entrypoint_install_phase_sec; no user packages.toml present). Full baked-stage matrix green post-trim (lab-matrix10): agents at /usr/local/bin, sweep, override shadow + fallback, doctor --fix volume cleanup. Removed formulae are one-line restores via packages.toml (docs/PACKAGES.md).
