...
- [x] Disk-backed named volume for `/home/linuxbrew` (mandatory: chown cost, §7.1) + volume equivalents for packages/home (`~/.config/construct-cli/home`) — LANDED 2026-08-19 in `backend_msb_run.go`: `EnsureMsbVolumes` (construct-packages = disk, construct-home = dir), `msbSandboxMounts` (volumes + project bind + conditional auto-mounts with RW qmd cache), `BuildMsbRunSpec` pure builder, `CreateMsbSandbox`. Unit-tested; not yet exercised against a live msb
