...
- **Terminal identity forwarding**: host terminal-identity markers (`KITTY_WINDOW_ID`, `GHOSTTY_RESOURCES_DIR`, `TERM_PROGRAM`) are passed into the container as `-e` flags on both launch paths (direct run and daemon, so `docker exec` sessions inherit them) so in-container TUIs and pi extensions detect the outer terminal (kitty-graphics inline images, etc.). `TERM` is excluded by default to avoid terminfo mismatch; add it to `env_passthrough` if needed.
- **Host loopback forwarding**: Chromium hardcodes `localhost`/`*.localhost` to `127.0.0.1` (RFC 6761), so a DNS-layer fix cannot let a headless browser (agent-browser) reach host dev sites. `entrypoint.sh` runs blind TCP relays on the container `127.0.0.1:<port>` -> `host.docker.internal:<port>` (default `[80, 443]` via `[sandbox] host_loopback_ports`), preserving the HTTP Host header and TLS SNI so host vhost routers/certs see the real hostname. The override grants `cap_add: NET_BIND_SERVICE` (consolidated with strict-mode `NET_ADMIN` into one block), and the entrypoint applies the socat file cap at runtime as root before the `gosu` drop (BuildKit blocks file-cap writes at build time).
- **Network modes**:
