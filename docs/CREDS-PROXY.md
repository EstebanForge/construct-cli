...
- Agent code opening a raw TCP connection to bypass the proxy. In **strict** and **offline** modes, msb egress policy blocks direct guest-to-internet connections, so the proxy is the only path. In **permissive** mode (default-allow egress) a hostile agent CAN reach the internet directly by ignoring `HTTPS_PROXY` — this is acceptable because the design's protection is that provider keys never enter the guest environment at all: there is nothing to exfiltrate. Permissive mode therefore protects keys, not egress; users who need egress control use strict mode.
...
NO_PROXY=construct-cli.local,localhost,127.0.0.1,host.microsandbox.internal
REQUESTS_CA_BUNDLE=/etc/ssl/certs/construct-ca.pem
NODE_EXTRA_CA_CERTS=/etc/ssl/certs/construct-ca.pem
```

The two CA-bundle variables are required because Python (`certifi`) and Node (`undici`) ignore the system trust store by default; without them the first provider request fails TLS verification against the construct CA.
...
The `NO_PROXY` list covers the local construct API and the `entrypoint.sh` loopback relays (`127.0.0.1:80/443`) so internal traffic does not round-trip through the proxy.
...
`internal/runtime/backend_msb_run.go::msbHostTransportRules` already permits guest-to-host transport. The proxy port must NOT be added to the `bridgePorts` argument of `BuildMsbRunSpec`: bridge ports are baked into boot-time egress rules, while the SSH and Herdr bridges bind random host ports at engine run time — whitelisting a fixed port there would switch the sandbox to a port-restricted host-TCP whitelist and break those bridges. Guest-to-host transport is already allowed; the proxy simply listens on a fixed host port within that allowance.

The proxy itself enforces the sandbox's network mode: it resolves the calling sandbox's mode and enforces mode policy itself — this is mandatory because the host-side proxy has full host internet access, and without mode enforcement an `offline` or `strict` agent could tunnel through the proxy and bypass its microVM egress filters. Concrete rules per mode:

- **permissive** (default-allow egress): the proxy brokers any allowlisted provider. Direct guest egress remains possible (permissive by definition) but carries no provider keys.
- **strict** (allowlist egress): the proxy only brokers providers whose hostnames are on the sandbox's egress allowlist; other provider requests are dropped with a policy error.
- **offline** (no egress): the proxy refuses all upstream connections with a clear "no upstream network" error; it never tunnels to the internet on the agent's behalf.
...
|---|---|---|
| `runtime.credential_proxy` | `false` (Phase 1-2), `true` (Phase 3+) | Enables the host-side proxy AND removes the env passthrough for the providers listed in `credential_proxy_providers`. |
| `runtime.credential_proxy_providers` | `[]` | Explicit list of brokered providers (e.g. `["anthropic", "openai"]`). Keys for listed providers are removed from the guest env and brokered by the proxy; keys for unlisted providers keep today's env passthrough. Per-provider enablement means an unbrokered provider cannot silently break. |
...
- P5.2 round (2026-09-21, isolated peer review): six findings adopted — the proxy port must NOT ride `bridgePorts` (would break random-port bridges), the proxy must enforce the sandbox network mode itself (offline/strict tunnel risk), permissive-mode direct egress acknowledged as keys-safe, `REQUESTS_CA_BUNDLE`/`NODE_EXTRA_CA_CERTS` added to guest wiring (certifi/undici ignore system CAs), `NO_PROXY` extended with `127.0.0.1` + `host.microsandbox.internal` (loopback relays), and the rollout flag split into a per-provider list.
