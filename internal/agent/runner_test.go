...
func TestReadKeyringEnv(t *testing.T) {
	// Create the directory structure: tmpDir/.config/construct-cli/home/
	tmpDir := t.TempDir()
	containerHome := filepath.Join(tmpDir, ".config", "construct-cli", "home")
	if err := os.MkdirAll(containerHome, 0755); err != nil {
		t.Fatal(err)
	}

	// Override UserHomeDir via HOME env
	t.Setenv("HOME", tmpDir)

	t.Run("parses valid keyring env file", func(t *testing.T) {
		envFile := filepath.Join(containerHome, ".construct-keyring-env")
		content := `# construct-managed: true - auto-generated by entrypoint
GNOME_KEYRING_CONTROL=/home/construct/.cache/keyring-ABC123
DBUS_SESSION_BUS_ADDRESS=unix:path=/tmp/dbus-XYZ789
`
		...
		if result["DBUS_SESSION_BUS_ADDRESS"] != "unix:path=/tmp/dbus-XYZ789" {
			t.Errorf("expected DBUS_SESSION_BUS_ADDRESS, got %v", result)
		}
	})

	t.Run("ignores unknown keys", func(t *testing.T) {
		envFile := filepath.Join(containerHome, ".construct-keyring-env")
		content := `GNOME_KEYRING_CONTROL=/foo
UNKNOWN_VAR=bar
DBUS_SESSION_BUS_ADDRESS=unix:path=/baz
`
		if err := os.WriteFile(envFile, []byte(content), 0644); err != nil {
			t.Fatal(err)
		}
