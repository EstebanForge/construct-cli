...
func (c *PackagesConfig) GenerateInstallScript() string {
	...
	script += "rm -f \"$HOME/.construct_patched\"\n\n"
	...
	script += "if command -v brew &> /dev/null; then\n"
	script += "    INSTALLED_BREW=$(brew list --formula -1 2>/dev/null || true)\n"

	script += "    echo 'Installing imagemagick (required for clipboard)...'\n"
	script += "    if echo \"$INSTALLED_BREW\" | grep -q \"^imagemagick$\"; then\n"
	script += "        echo \"  ✓ imagemagick already installed\"\n"
	script += "    else\n"
	script += "        brew install --formula imagemagick || echo \"⚠️ Failed to install imagemagick\"\n"
	script += "    fi\n\n"

	script += "    echo 'Installing topgrade (system updater)...'\n"
	script += "    if echo \"$INSTALLED_BREW\" | grep -q \"^topgrade$\"; then\n"
	script += "        echo \"  ✓ topgrade already installed\"\n"
	script += "    else\n"
	script += "        brew install --formula topgrade || echo \"⚠️ Failed to install topgrade\"\n"
	script += "    fi\n\n"

	script += "    echo 'Installing libgit2 (dependency for cargo-update)...'\n"
	script += "    if echo \"$INSTALLED_BREW\" | grep -q \"^libgit2$\"; then\n"
	script += "        echo \"  ✓ libgit2 already installed\"\n"
	script += "    else\n"
	script += "        brew install --formula libgit2 || echo \"Warning: libgit2 installation failed\"\n"
	script += "    fi\n"
	script += "else\n"
	script += "    echo \"⚠️ Homebrew not found; skipping core brew packages\"\n"
	script += "fi\n\n"
	...
	if len(c.Brew.Taps) > 0 || len(c.Brew.Packages) > 0 {
		...
		if len(c.Brew.Packages) > 0 {
			script += "    # Get list of installed formulae to avoid noise\n"
			script += "    INSTALLED_BREW=$(brew list --formula -1 2>/dev/null || true)\n"
			for _, pkg := range c.Brew.Packages {
				// Use --formula to avoid disambiguation errors with tap-qualified names
				script += "    if echo \"$INSTALLED_BREW\" | grep -q \"^" + pkg + "$\"; then\n"
				script += "        echo \"  ✓ " + pkg + " already installed\"\n"
				script += "    else\n"
	...
	script += "missing_cmds=\"\"\n"
	script += "for cmd in claude amp copilot opencode qwen cline crush codex goose gemini kilocode pi; do\n"
