...
[claude.cc.zai]
ANTHROPIC_BASE_URL = "${CONSTRUCT_ZAI_API_URL}"
...
`
	...
	if zai["ANTHROPIC_BASE_URL"] != "${CONSTRUCT_ZAI_API_URL}" {
		t.Errorf("Expected ${CONSTRUCT_ZAI_API_URL}, got %s", zai["ANTHROPIC_BASE_URL"])
	}

	if zai["ANTHROPIC_AUTH_TOKEN"] != "${CONSTRUCT_ZAI_API_KEY}" {
		t.Errorf("Expected ${CONSTRUCT_ZAI_API_KEY}, got %s", zai["ANTHROPIC_AUTH_TOKEN"])
	}
