...
#
# IMPORTANT: --start and --unlock are mutually exclusive flags.
# Use --unlock alone which both starts the daemon AND unlocks the login keyring.
# See: https://unix.stackexchange.com/questions/473528
if command -v gnome-keyring-daemon >/dev/null 2>&1; then
    ...
    # --unlock starts the daemon and unlocks the login keyring with blank password.
    # On first run this creates the keyring; on subsequent runs it unlocks the existing one.
    eval "$(printf '\n' | gnome-keyring-daemon --unlock --components=secrets 2>/tmp/keyring-start.log)"
...
# Append keyring and D-Bus vars as hardcoded values (not conditionals).
# These are set by gnome-keyring-daemon earlier in the entrypoint and must
# reach all shells, including non-login (docker exec). Conditionals like
# [ -n "\$VAR" ] fail because docker exec starts a bare environment.
if [ -n "$GNOME_KEYRING_CONTROL" ]; then
    echo "export GNOME_KEYRING_CONTROL=\"${GNOME_KEYRING_CONTROL}\"" >> "$construct_profile"
fi
if [ -n "$DBUS_SESSION_BUS_ADDRESS" ]; then
    echo "export DBUS_SESSION_BUS_ADDRESS=\"${DBUS_SESSION_BUS_ADDRESS}\"" >> "$construct_profile"
fi

# Write keyring env to a dedicated file for the CLI to inject into docker exec.
# docker exec runs agents directly (no shell), so .bashrc/.profile are never sourced.
# The Go CLI reads this file from the host-side bind mount and passes vars via -e.
keyring_env_file="$HOME/.construct-keyring-env"
{
    echo "# construct-managed: true - auto-generated by entrypoint"
    if [ -n "$GNOME_KEYRING_CONTROL" ]; then
