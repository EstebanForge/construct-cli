...
- **Environment Variables**: All use `CONSTRUCT_<PROVIDER>_API_KEY` naming convention

**Supported Providers:**
- **Z.AI** (`zai`) - `CONSTRUCT_ZAI_API_KEY`
- **MiniMax** (`minimax`) - `CONSTRUCT_MINIMAX_API_KEY`
- **Kimi** (`kimi`) - `CONSTRUCT_KIMI_API_KEY`
- **Qwen** (`qwen`) - `CONSTRUCT_QWEN_API_KEY`
- **Mimo** (`mimo`) - `CONSTRUCT_MIMO_API_KEY`
...
- **First run** (`construct sys init`):
  ...
  - Sets up Claude provider configuration template with `CONSTRUCT_` prefixed environment variables.
...
- Environment variable naming convention (`CONSTRUCT_` prefix) should be consistently applied to prevent conflicts.
