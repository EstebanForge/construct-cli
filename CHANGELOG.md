...
### Added
- **Host exec bridge now propagates the agent's working directory to host binaries**: when an agent invokes an allowlisted host binary (`host_binaries`, e.g. `wicket`) from inside the sandbox, the bridge previously ran it in the daemon's cwd. So cwd-aware CLIs like `wicket wp` (which detect the Docker Compose project by walking up from cwd for a compose file, with no `--project` flag) resolved the wrong project and failed with `no services found`. The shim (`internal/templates/construct-host-exec`) now sends the container `$PWD` as a `cwd` field on the `/exec` payload; `internal/hostexec/server.go` translates it to the matching host path via any construct bind mount (the project mount `${PWD}:${CONSTRUCT_PROJECT_PATH}` plus daemon multi-path `/workspaces` mounts), validates the result stays at or under that mount's host root (rejecting `../` escapes and arbitrary host dirs like `/etc`), and sets it as the child's working directory. Empty, unset, or out-of-mount cwd falls back to inheriting the daemon cwd (the previous behavior), so the change is backward compatible. `internal/agent/engine.go` passes the project mount plus every resolved daemon multi-path mount (`runtime.ResolveDaemonMounts`) to `StartServer` as a list of container↔host path maps. After this, an agent can `cd` into a client project (including one reached through a daemon `/workspaces` mount) and run `wicket wp` / `wicket docker` and have them target that project on the host. Security: cwd is constrained to construct's own bind mounts; anything outside their host roots is rejected, which adds nothing the agent could not already do. v1 limitation: only construct-created mounts are translated (project + daemon multi-path), not the home mount (`/home/construct`); a cwd there falls back to inheriting. Requires `construct build` to bake the updated shim into the image. Documented in `docs/HOST-EXEC.md`.
<!-- RELEASE:END 1.14.1 -->
